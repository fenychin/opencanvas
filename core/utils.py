...
from core.database import HISTORY_LOCK
...
def convert_output_to_jpg(url, quality=88):
    ...
    try:
        ...
        try:
            root = config.ASSETS_DIR if os.path.commonpath([os.path.abspath(config.ASSETS_DIR), os.path.abspath(jpg_path)]) == os.path.abspath(config.ASSETS_DIR) else config.OUTPUT_DIR
        except ValueError:
            root = config.OUTPUT_DIR
        rel = os.path.relpath(jpg_path, root).replace("\\", "/")
        prefix = "/assets" if root == config.ASSETS_DIR else "/output"
        return f"{prefix}/{rel}"
