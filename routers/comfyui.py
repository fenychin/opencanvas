...
from core.database import GLOBAL_CONFIG_LOCK
...
def workflow_path_from_name(name: str) -> str:
    ...
    path = os.path.abspath(os.path.join(config.WORKFLOW_DIR, *name.split("/")))
    workflow_root = os.path.abspath(config.WORKFLOW_DIR)
...
def save_comfyui_instances(payload: ComfyInstancesPayload):
    ...
    try:
        config.update_env_values({"COMFYUI_INSTANCES": ",".join(cleaned)})
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"写入 env 失败：{e}")
    
    config.COMFYUI_INSTANCES = cleaned
    config.COMFYUI_ADDRESS = cleaned[0]
    
    new_load = {addr: 0 for addr in cleaned}
    for addr, n in (config.BACKEND_LOCAL_LOAD or {}).items():
        if addr in new_load:
            new_load[addr] = n
    config.BACKEND_LOCAL_LOAD = new_load
    return {"instances": config.COMFYUI_INSTANCES}
...
def list_workflows():
    if not os.path.isdir(config.WORKFLOW_DIR):
        return {"workflows": []}
    items = []
    for root, dirs, files in os.walk(config.WORKFLOW_DIR):
        if os.path.abspath(root) == os.path.abspath(config.WORKFLOW_DIR):
        ...
        for fn in sorted(files):
            if not fn.endswith(".json") or fn.endswith(".config.json"):
                continue
            rel = os.path.relpath(os.path.join(root, fn), config.WORKFLOW_DIR).replace("\\", "/")
...
def upload_workflow(payload: WorkflowUploadRequest):
    ...
    custom_dir = os.path.join(config.WORKFLOW_DIR, CUSTOM_WORKFLOW_FOLDER)
...
@router.put("/workflows/{name:path}/config")
...
@router.delete("/workflows/{name:path}")
...
@router.post("/workflows/{name:path}/run")
